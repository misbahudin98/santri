...
class SessionManager extends Controller
{
    ...
    public static function santri()
    {
        ...
        $data = new Controller;
        $hasil = $data->model('User_model')->login_santri();

        if (is_array($hasil)) {

            $hasil['session'] = Security::pass_hash(bin2hex(random_bytes(255)));

            $data->model('User_model')->set_session_santri($hasil);
            $jwt = self::jwt_encode($hasil, $hasil['session']);
            self::set_cookies($jwt);
            ...
            try {
                ...
                if ($hasil) {
                    ...
                    if ($now < $expired) {
                        ...
                        //in_array($class[0], $access)
                        if (!empty($access) &&  in_array('admin', $access) ) {
    ...
    public static function getCurrentSantri()
    {
        if (isset($_COOKIE[self::$cookie_name])) {
            try {
                $payload = self::jwt_decode();
                $data = new Controller;
                $hasil = $data->model('User_model')->session_santri([$payload['id'], $payload['nama']]);
                if ($hasil) {
                    $expired = (strtotime($hasil['last_access'] . " +" . self::$cookie_expired . " second"));                    
                    $now = strtotime("Y-m-d H:i:s");
                    if ($now < $expired) {
            ...
            try {
                ...
                if ($hasil) {
                    ...
                    if ($now < $expired) {
                        ...
                        $class = self::class();
                        // var_dump( in_array($class[0], $access)) or die;
    ...
    public static function logout_santri()
    {
        if (isset($_COOKIE[self::$cookie_name])) {
            $url = self::url();
            $option = [
            ...
            $data = new Controller;
            $payload = self::jwt_decode();
            $data->model('User_model')->delete_session_santri($payload);
            setcookie(self::$cookie_name, '', $option);
        }
