...
$data = SessionManager::getCurrentSantri();

class Santri extends Controller
{


    private  $subdomain = "santri";

    public function index()
    {

        $this->santri("assalamu'alaikum-warohmatullohi-wabarokatuh", "success");
    }
    ...
    public function akses($pesan = '', $tipe = '')
    {
        ...
        $data['table'] = true;
        $data['button'] = true;
        $data['session'] =SessionManager::getCurrentSantri();

        if (!empty($pesan) && !empty($tipe)) {

            flasher::setFlash($pesan, $tipe);
        }
        ...
        $output .= flasher::input("", 'akses', "select", "required", "", "", ['santri' => 'santri', 'Logistik' => 'logistik', 'Konten' => 'konten', 'Marketing' => 'marketing', 'Customer Service' => 'cs', 'Penjualan' => 'penjualan', 'Keuangan' => 'keuangan', 'Pendapatan' => 'pendapatan']);
        $data['output']  = $output;

        $data['column'] = "[{title:'Aksi'},  {title:'username'},
        {title:'akses'},{title:'Terakhir diupdate'},{title:'Status Akses'}]";
        ...
        $this->view('santri/header', $data);
        $this->view('santri/crud', $data);

        $this->view('santri/footer', $data);
    }
    ...
    public function tambah()
    {
        ...
        if ($this->model('User_model')->tambah() > 0) {
            header("Location:" . BASEURL . "santri/santri/berhasil-menambah-data!/success");
            exit();
        } else {
            header("Location:" . BASEURL . "santri/santri/Gagal-menambah-data!/error");
            exit();
        }
    ...
    public function tambah_akses()
    {
        ...
        if ($this->model('User_model')->tambah_akses() > 0) {
            header("Location:" . BASEURL . "santri/akses/berhasil-menambah-data!/success");
            exit();
        } else {
            header("Location:" . BASEURL . "santri/akses/Gagal-menambah-data!/error");
            exit();
        }
    }
    public function tambah_detail()
    {
        ...
        if ($this->model('User_model')->tambah_detail() > 0) {
            header("Location:" . BASEURL . "santri/detail/berhasil-menambah-data!/success");
            exit();
        } else {
            header("Location:" . BASEURL . "santri/detail/Gagal-menambah-data!/error");
            exit();
        }
    }
    ...
    public function edit_akses()
    {
        ...
        $output .= flasher::input($user['akses'], 'akses', "select", "required", "", "", ['santri' => 'santri', 'Logistik' => 'logistik', 'Konten' => 'konten', 'Marketing' => 'marketing', 'Customer Service' => 'cs', 'Penjualan' => 'penjualan', 'Keuangan' => 'keuangan', 'Pendapatan' => 'pendapatan']);
    ...
    public function update()
    {
        ...
        if ($this->model('User_model')->update() > 0) {
            header("Location:" . BASEURL . "santri/santri/berhasil-mengubah-data!/success");
            exit();
        } else {
            header("Location:" . BASEURL . "santri/santri/Tidak-ada-data-yang-berubah!/error");
            exit();
        }
    }
    ...
    public function update_akses()
    {
        ...
        if ($this->model('User_model')->update_akses() > 0) {
            header("Location:" . BASEURL . "santri/akses/berhasil-mengubah-data!/success");
            exit();
        } else {
            header("Location:" . BASEURL . "santri/akses/Tidak-ada-data-yang-berubah!/error");
            exit();
        }
    }
    ...
    public function update_detail()
    {
        if ($this->model('User_model')->update_detail() > 0) {
            header("Location:" . BASEURL . "santri/detail/berhasil-mengubah-data!/success");
            exit();
        } else {
            header("Location:" . BASEURL . "santri/detail/Tidak-ada-data-yang-berubah!/error");
            exit();
        }
    }
}
